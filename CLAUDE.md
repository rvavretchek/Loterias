...
Django web app that generates bet combinations for six Brazilian lotteries (Mega-Sena, +Milionária, Lotomania, Lotofácil, Quina, Dupla-Sena). Single-tenant, multi-user: every user shares one SQLite database — there is no per-organization isolation (an earlier iteration added SQLite-per-tenant multitenancy; it was removed on 2026-09-07 because it was architecturally wrong for this app and was the root cause of a "can't generate games" bug — see [docs/diagnostico-projeto.md](docs/diagnostico-projeto.md) for the full diagnosis).

**Naming convention (changed 2026-09-08):** code identifiers — variables, constants, classes, functions, model fields — are in English going forward, including a planned rename of existing Portuguese identifiers (`JogoGerado` → `GeneratedBet`, `ResultadoLoteria` → `LotteryResult`, `capturar_resultado_cef` → `fetch_cef_result`, etc.). UI strings, `verbose_name`/labels, and the Portuguese domain vocabulary (Jogo, Concurso, Acerto, ...) are unaffected and stay pt-br. The full old→new mapping and rationale live in the PRD's naming section ([_bmad-output/planning-artifacts/prds/prd-Loterias-2026-09-07/prd.md](_bmad-output/planning-artifacts/prds/prd-Loterias-2026-09-07/prd.md) §3.1); as of this PRD the rename itself had not yet been executed in code — the "Lottery domain logic" section below still describes the current (Portuguese) identifiers. Treat any new code you write as English-first; check whether the legacy names below have since been renamed before relying on them.
