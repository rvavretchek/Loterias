...
Tema escuro aplicado desde o Epic 7 (Story 7.4, 2026-09-24): tokens `--dark-*` em `lottiq-tokens.css` sob `:root[data-theme="dark"]`, remapeando os semânticos (`--surface-*`, `--text-*`, `--border-*`, `--action-*`, `--status-*`, `--ball-*`). `base.html` aplica `data-theme="{{ theme }}"` no `<html>` via `theme_context` (`apps/loterias_core/context_processors.py`) — `User.preferred_theme` pro autenticado, sessão pro anônimo; alternador de volta no cabeçalho (`toggle_theme`, `apps/accounts/views.py`). **Ao criar cor nova, sempre use o token semântico (`var(--text-heading)`, `var(--action-primary)`, `var(--status-waiting-fg)`, ...), nunca a paleta bruta (`var(--teal-700)`, `var(--lq-navy)`) direto num componente de UI** — a paleta bruta não muda com o tema; só os semânticos têm par claro/escuro. Essa confusão (bruta em vez de semântica) foi a causa de vários elementos "não acompanharem" o tema escuro numa rodada de QA (nav ativo ilegível, bolinhas de número quase invisíveis, botões secundários com cor de texto fixa) — corrigido trocando cada ocorrência de `--teal-700`/`--lq-navy` usada como cor de texto/ícone pelo token semântico equivalente.
...
- `generate_bet()` (wrapper de 2 valores) / `generate_bet_with_relaxation()` — geram um jogo válido aleatório e devolvem `(numeros, trevos, regra_relaxada)`. Sem linhas `GenerationRule` do usuário+Jogo vale o modo padrão: jogos em `GAMES_WITH_SEQUENCE_RULE` (Mega-Sena, +Milionária, Quina, Dupla-Sena) seguem a regra de sequência adaptativa (se os últimos `MIN_SEQUENCE_INTERVAL` (5) jogos daquele tipo tiveram par sequencial, o próximo evita pares por completo; senão, no máximo um; `count_sequential_pairs()` conta). Com qualquer linha `GenerationRule` (mesmo desligada), só as regras ligadas valem e a adaptativa deixa de ser consultada. Se 10000 tentativas falham, relaxa UMA regra (a de maior `updated_at` entre as violadas pelo último candidato), só em memória, e tenta mais 10000; senão devolve `(None, None, None)`. Views e API avisam a regra relaxada (API: campo `regra_relaxada`).
- `bet_satisfies_rules()` — função pura que checa um candidato contra as regras e devolve `(ok, regras_violadas)`. Regras (definidas em `RULE_DEFINITIONS`/`RULE_NAMES_BY_GAME` em `models.py`): tamanho máximo de sequência, máximo de sequências, máximo por linha/coluna do volante (`GAME_GRID`), espaço mínimo entre sequências (números fora do jogo entre sequências vizinhas), mínimo de sequências e Distribuição Homogênea (1 número por faixa; Lotofácil = 3 por linha). Lotomania só tem as 3 regras de sequência. Editadas em `/regras/<jogo>/`; salvar nunca apaga linhas, só "Restaurar padrão".
...
Models: `GenerationRule` (uma linha por usuário+Jogo+regra: `enabled`, `numeric_value`/`choice_value`), `GeneratedBet` (um jogo gerado pelo usuário, FK pro `AUTH_USER_MODEL`), `LotteryResult` (resultado oficial do sorteio, único por `game`+`contest`), `GameStatistics` (estatísticas agregadas por usuário/jogo, único por `user`+`game`).
