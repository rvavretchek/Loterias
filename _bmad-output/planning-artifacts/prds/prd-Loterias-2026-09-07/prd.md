...
`apps/accounts` já segue a convenção (classes `User`, `UserManager`, `CustomSignupForm`, `CustomAccountAdapter` já em inglês) — não entra no escopo de renomeação, só o novo código do fluxo de cadastro (FR-10 a FR-14) precisa nascer consistente com isso.
