...
- E `GeneratedBet` sem interseção nenhuma **e sem prêmio real** não gera Notificação (corrigido pós-fechamento, ver Notas de Implementação)
...
Isso não resolvia por completo o caso da Lotomania: o gatilho de notificação em `_notify_covered_bets` continuava exigindo `hits > 0`, porque o Critério de Aceite (e a FR-3 da PRD, sua fonte original) dizia literalmente "GeneratedBet sem interseção nenhuma não gera Notificação", sem ressalva. Documentei isso como decisão de produto pendente em vez de decidir sozinho.

**Correção pós-fechamento (mesmo dia, a pedido do Boss):** a decisão foi resolvida com uma diretriz permanente pro projeto -- "Uma regra de negócio NUNCA pode quebrar na vida real. Se ela quebra é porque a regra está errada; não está modelando a realidade." A Lotomania paga por 0 acertos na vida real, então a regra (PRD FR-3, epics.md Story 2.3, e este Critério de Aceite) estava errada, não o código. Corrigido na origem: PRD FR-3, `epics.md` (Story 2.3), e o gatilho de notificação em `jobs._notify_covered_bets`, que agora é `if prize['hits'] > 0 or prize['won']:` -- cobre tanto acerto parcial (hits>0, premiado ou não) quanto o prêmio real de 0 acertos da Lotomania (hits=0, won=True). Teste novo (`test_lotomania_zero_hits_prize_generates_notification`) confirma. O item de `deferred-work.md` sobre essa tensão foi removido (resolvido, não mais pendente).
