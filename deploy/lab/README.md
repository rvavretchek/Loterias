...
Ate essas duas coisas serem corrigidas no Salt (o `.sls` fica no master `ubt-host02`, sem acesso SSH daqui), **contornado em `.github/workflows/deploy.yml` (2026-09-14):** um segundo step no workflow roda `sudo salt 'ubt-host01' cmd.run '... build --no-cache && up -d --remove-orphans && nginx -s reload'` -- execucao ad hoc via Salt (o runner self-hosted e o proprio `ubt-host02`, que ja tem `sudo salt` funcionando pro state.apply), nao passa pelo state `subir_Loterias` quebrado, entao um push pra `main` volta a redeployar de verdade sem intervencao manual. Se o `.sls` for corrigido no master no futuro, esse step de contorno pode ser removido do workflow.
